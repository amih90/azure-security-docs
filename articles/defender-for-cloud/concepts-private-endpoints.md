...
You can use private endpoints with your Microsoft Security Private Links to allow workloads on a virtual network to securely access Microsoft Defender services over a Private Link. The private endpoint uses an IP address from the virtual network address space to access Microsoft Defender services. Network traffic between your workloads and Microsoft Defender services traverses over the virtual network and a private link on the Microsoft backbone network, elliminating exposure to the public internet while enabling security protection for workloads in network-isolated environments.

Using private endpoints with Microsoft Security Private Links enables you to:
...
> [!IMPORTANT]
> - For network-isolated workloads, Microsoft Security Private Links replace the need of Azure Monitor Private Link Scope (AMPLS) and Azure Firewall egress rules.
...
Workload owners and security administrators can manage consent requests and the private endpoints through the **Private endpoints connections** tab for the Security Private Llink in the Azure portal.
...
When you create a private endpoint, by default, a [private DNS zone](/azure/dns/private-dns-overview) is been provisioned and corresponds to the Microsoft Defender private link subdomain `*.defender.microsoft.com`.
...
| Defender for Containers | `*.privatelink.cloud.defender.microsoft.com` | CNAME | `<Defender public endpoint>` | 443 |
...
This approach enables workloads to access Microsoft Defender services on the virtual network hosting the private endpoints, and workloads outside the virtual network.

If you're using a custom DNS server on your network, clients must be able to resolve the FQDN for the Microsoft Defender services endpoints to the private endpoint IP address. You should configure your DNS server to delegate your private link subdomain to the private DNS zone for the virtual network, or configure the A records with the private endpoint IP address.
